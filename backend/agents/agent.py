...
When using the 'answer_jira_query' tool, you must ask the user for the 'issue_key' and the 'query' (e.g., "when can i expect TESTPROJ-10 to be complete" or "why is TESTPROJ-10 stuck") as they are required arguments for that tool.
 ...
 For ETA prompts like "When can I expect <ISSUE-KEY> will be done?", call cpa_engine_agent's 'estimate_issue_eta_days(issue_key)', then pass the result to formatter_agent for proper UI formatting.
 For queries about issues assigned to a specific user (e.g., "how many tasks do I have assigned?", "show me issues assigned to USER_A"), use jira_sprint_agent's 'get_issues_assigned_to_user(username)' tool, then pass the raw result to formatter_agent for proper UI formatting.
 ...
 - Pass the tool's output to formatter_agent for proper UI formatting.

 Generative UI directive:
 - When the user asks for the Jira status for a specific issue (e.g., "JIRA status for issue PROJ-123"), use formatter_agent's 'format_jira_status(issue_key)' tool to generate the proper UI response.
 - When the user asks who is assigned to a specific issue (e.g., "who is assigned TESTPROJ-16", "assignee for PROJ-123"), call the direct 'who_is_assigned(issue_key)' tool, then pass the result to formatter_agent for proper UI formatting.
 - When the user asks for the list of Jira issues in the current sprint (e.g., "JIRA issues in the current sprint"), get the raw data from appropriate tools, then use formatter_agent to format the response properly.
 - Do not include analysis or extra text around the JSON. If the issue key is unclear, ask a clarifying question in plain text.
    """,
    sub_agents=[jira_agent, github_repo_agent, jira_cpa_agent, cpa_engine_agent, formatter_agent],
    tools=[
